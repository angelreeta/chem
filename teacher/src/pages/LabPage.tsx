...
type MobileModal = 'tutor' | 'calc' | 'notes' | null;
    ...
    <div className="flex flex-col w-full h-full bg-gray-950 overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          {/* Step controls - Hidden on mobile when modal is open */}
          <div className={`border-t border-slate-800 bg-slate-900/80 p-4 space-y-3 shrink-0 ${mobileModal ? 'hidden sm:block' : ''}`}>
            {completed ? (
              ...
              <>
                ...
                <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap">
                  ...
                  <button onClick={() => setActiveTab('notes')} className="hidden lg:flex items-center gap-1.5 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-400 border border-slate-700 rounded-lg text-xs font-medium transition-colors">
                    <NotebookPen className="w-3.5 h-3.5" />
                    Notes
                  </button>
                  <button onClick={() => setActiveTab('calc')} className="hidden lg:flex items-center gap-1.5 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-400 border border-slate-700 rounded-lg text-xs font-medium transition-colors">
                  ...
                  <button onClick={handleNextStep} className="ml-auto flex items-center gap-1.5 px-4 py-2 bg-cyan-500 hover:bg-cyan-400 text-gray-900 rounded-lg text-sm font-semibold transition-colors">
                    {currentStep < steps.length - 1 ? <><ChevronRight className="w-4 h-4" /><span className="hidden xs:inline">Next Step</span></> : <><CheckCircle className="w-4 h-4" /><span className="hidden xs:inline">Complete</span></>}
                  </button>
                </div>
              </>
            )}
        ...
        <div className="fixed bottom-0 left-0 right-0 z-50 lg:hidden flex items-center justify-around bg-slate-900/95 backdrop-blur-md border-t border-slate-800 px-2 py-1.5">
          {TABS.map(({ id, label, icon: Icon }) => (
            <button
              key={id}
              onClick={() => setMobileModal(id)}
        ...
        {/* Mobile Bottom Modal Sheets */}
        <div className={`fixed inset-0 z-40 lg:hidden ${mobileModal ? 'block' : 'hidden'}`}>
          {/* Backdrop */}
          <div
          ...
          />
          {/* Modal content */}
          <div className="absolute bottom-0 left-0 right-0 max-h-[85vh] flex flex-col bg-slate-900 rounded-t-2xl border-t border-slate-700 shadow-2xl">
            {/* Handle bar */}
            ...
            <div className="flex items-center justify-between px-4 pb-3 border-b border-slate-800">
              <h3 className="text-white font-semibold">
                {mobileModal === 'tutor' && 'AI Tutor (Aria)'}
                {mobileModal === 'calc' && 'Chemistry Calculations'}
                {mobileModal === 'notes' && 'Observation Notes'}
              </h3>
              <button
                onClick={() => setMobileModal(null)}
            ...
            {mobileModal === 'notes' && (
              <div className="flex-1 flex flex-col overflow-hidden min-h-0">
                ...
                <textarea
                  ...
                  className="flex-1 bg-transparent border-0 resize-none text-slate-200 text-xs p-4 placeholder-slate-600 focus:outline-none leading-relaxed font-mono min-h-0"
                />

                <div className="px-4 py-2.5 border-t border-slate-800 flex items-center justify-between shrink-0 pb-20">
        ...
        <div className="hidden lg:flex w-72 xl:w-80 flex-col border-l border-slate-800 bg-slate-900 shrink-0">
          {/* Tab header */}
